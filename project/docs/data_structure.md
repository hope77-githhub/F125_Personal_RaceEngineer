# F1 26 UDP | Library for use on ESP 32 / ESP8266 devices
**Data Output from F1® 26 Game**

This program captures and parses packets that are sent by the F1 26 game. This library is written specifically for usage on the ESP32 and ESP8266.
...
struct PacketHeader
{
    ...
    uint32 m_overallFrameIdentifier;   // Overall identifier for the frame the data was retrieved on,
...
|Car Telemetry 2|16|Additional telemetry data for all cars (active aero, overtake)|
...
*N.B. For the quantised g-Force values, divide by 1000.0f to get the actual value.*
...
struct PacketSessionData
{
    ...
    uint8                m_weekendStructure[12];                          // List of session types to show weekend structure - see appendix
...
struct CarStatusData
{
    ...
    uint8  m_ersDeployMode;            // ERS deployment mode, 0 = none, 1 = medium, 2 = hotlap, 3 = boost
...
struct LapHistoryData
{
    ...
    uint16 m_sector2TimeMSPart;     // Sector 2 milliseconds part
    uint8  m_sector2TimeMinutes;    // Sector 2 whole minute part
    uint16 m_sector3TimeMSPart;     // Sector 3 milliseconds part
...
This packet provides additional telemetry data specific to F1 26 regulations, including active aero status and overtake mode availability.
...
struct CarTelemetry2Data
{
    ...
    uint16 m_activeAeroActivationDistance; // 0 = Active aero not available,
                                           // non-zero - Active aero will be available in [X] metres
    uint8  m_overtakeAvailable;            // 0 = not available, 1 = available
    uint8  m_overtakeActive;               // 0 = not active, 1 = active
    uint16 m_overtakeActivationDistance;   // 0 = Overtake Mode not available,
                                           // non-zero - Overtake Mode will be available in [X] metres
...
{
    PacketHeader      m_header;                    // Header
    CarTelemetry2Data m_carTelemetry2Data[24];     // data for all cars on track
};
```
...
F1® 26 sees the following changes to the UDP specification:

- Increasing the maximum number of vehicles to 24 to accommodate the new
...
| 192 | Gabrielle Mini         | 193 | Sebastián Montoya       | 194 | Alex Dunne             |
| 195 | Max Esterson           | 196 | Sami Meguetounif        | 197 | John Bennet     |
